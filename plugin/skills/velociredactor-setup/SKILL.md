...
# Setting up Veloci Redactor for a project

The goal is an `agent:` section in the project's `veloci.yml` that lists the files agents must read through `veloci redact`. **The user decides what goes in it.** Your job is to find likely candidates, ask, and record the answer.
...
`veloci agent status` already lists two kinds of candidates. Add `--json` for machine-readable output (`suggested` and `secrets`).
...
veloci scan --json       # machine-readable
```

Treat both lists as a starting point. Also look at the names of tracked and ignored files yourself, for data that detectors don't recognize, such as customer exports. **Don't read file contents to judge them.** `scan` and `veloci list FILE` tell you what's inside without showing it.
...
3. **Enforcement?** In agents that support hooks, including Claude Code with the Veloci Redactor plugin, `enforce: true` *blocks* direct reads of protected files, and searches that would reach them, instead of relying on instructions alone. The agent is pointed at `veloci redact` or `veloci grep` instead. Recommend it for repositories with real secrets.
...
Pattern syntax follows `.gitignore`, relative to the directory holding `veloci.yml`:
...
```sh
veloci agent status
veloci agent check <one file per category>   # each should print and exit 1
