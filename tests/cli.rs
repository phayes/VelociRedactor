...
fn allow_and_check() {
    ...
    let out = redact(
        &["--check", "--allow-value", S, "--allow-value", "hunter2"],
        &input,
    );
...
fn unmatched_allow_entries_warn() {
    let out = redact(&["--allow-value", "nope"], "DB_PASSWORD=hunter2");
    let err = stderr(&out);
    assert!(
...
fn list_shows_token_start_and_length_but_not_values() {
    ...
    let out = list(&["--allow-value", S], &format!("x {S}\n"));
...
fn custom_rules_and_packs() {
    ...
    let listed = list(
        &["--json", "--rules-pack", dir.path().to_str().unwrap()],
        "ref TEAM-123\n",
    );
    let doc: serde_json::Value = serde_json::from_slice(&listed.stdout).unwrap();
...
fn custom_ruleset_replaces_bundled_rules() {
    ...
    let listed = list(
        &["--json", "--ruleset", path.to_str().unwrap()],
        "ZZ1234 ghp_a1b2c1d2e1f2g1h2a1b2c1d2e1f2g1h2a1b2\n",
    );
    let doc: serde_json::Value = serde_json::from_slice(&listed.stdout).unwrap();
