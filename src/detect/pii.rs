...
impl Pii {
    ...
    /// The detector for this category.
    pub fn detector(self) -> Box<dyn Detector> {
        match self {
            Pii::Email => Box::new(EmailDetector),
...
fn is_allowlisted_email(email: &str) -> bool {
    let lower = email.to_lowercase();
    EMAIL_ALLOWLIST.iter().any(|pattern| {
...
impl Detector for EmailDetector {
    ...
    fn detect(&self, value: &str, _ctx: &LeafContext<'_>, out: &mut Vec<Detection>) {
        for m in EMAIL.find_iter(value) {
            if !is_allowlisted_email(m.as_str()) {
